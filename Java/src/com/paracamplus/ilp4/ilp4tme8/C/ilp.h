...
/** TME 8 : The structure of ILP_Property **/
...
typedef struct ILP_Object {
     ...
     ILP_Property*      properties;           // Ajout de propriétés dans la structure d'ILP_Object
     union {
...
typedef struct ILP_Exception {
     struct ILP_Class* _class;
     ILP_Property       properties;    // Ajout de propriétés dans ILP_Exception
     union {
          struct asException_ {
               char                message[ILP_EXCEPTION_BUFFER_LENGTH];
               struct ILP_Object*  culprit[ILP_EXCEPTION_CULPRIT_LENGTH];
...
typedef struct ILP_Class {
     struct ILP_Class* _class;
ILP_Property       properties;   // Ajout aussi dans ILP_Class
     union {
          struct asClass_ {
               struct ILP_Class*   super;
               char*               name;
...
typedef struct ILP_Method {
     struct ILP_Class* _class;
ILP_Property       properties;     // de même
     union {
          struct asMethod_ {
               struct ILP_Class*   class_defining;
               char*               name;
